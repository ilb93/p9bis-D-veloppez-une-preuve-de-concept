"""
Version locale du dashboard SAINT - Charge depuis les fichiers locaux au lieu de S3
Utilisez cette version pour tester sans credentials AWS
"""
...
# Importer le modèle SAINT
from streamlit_app.saint_model import SAINTModel, predict_saint, load_saint_model_from_files
...
# ======================================================
# CHARGEMENT MODÈLE SAINT DEPUIS FICHIERS LOCAUX
# ======================================================
@st.cache_resource
def load_saint_model_local():
    """Charge le modèle SAINT depuis les fichiers locaux"""
    try:
        # Chemin vers les fichiers locaux (dans le dossier parent)
        base_path = Path(__file__).parent.parent
        
        files = {
            "weights": base_path / "saint_weights.pth",
            "config": base_path / "saint_config.pkl",
            "metadata": base_path / "saint_metadata.pkl",
            "threshold": base_path / "saint_threshold.pkl"
        }
        ...
        if missing_files:
            ...
            st.info(f"💡 Assurez-vous que les fichiers SAINT sont dans: {base_path}")
            st.stop()
        
        # Charger le modèle
        model_data = load_saint_model_from_files(
            weights_path=str(files['weights']),
            config_path=str(files['config']),
            metadata_path=str(files['metadata']),
            threshold_path=str(files['threshold']) if files['threshold'].exists() else None,
            device='cpu'
        )
...
# Charger le modèle
with st.spinner("🔄 Chargement du modèle SAINT depuis les fichiers locaux..."):
    try:
        model_data = load_saint_model_local()
...
if EXPECTED_FEATURES and len(EXPECTED_FEATURES) > 0:
    ...
    st.warning("⚠️ Aucune feature disponible pour la prédiction.")
